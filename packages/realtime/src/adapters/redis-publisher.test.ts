...
function makeGoalEvent(overrides: Partial<GoalEvent> = {}): GoalEvent {
  return {
    matchId: "match-1",
    teamId: "team-1",
    homeScore: 1,
    awayScore: 0,
    scoredAt: "2026-08-15T10:00:00.000Z",
    ...overrides,
  };
}
...
describe("RedisPublisher.publishGoal", () => {
  it("publishGoal() gọi đúng kênh global cố định 'goal-events' (KHÔNG per-match), kèm JSON payload đúng", async () => {
    const publishMock = vi.fn().mockResolvedValue(1);
    const publisher = new RedisPublisher({
      redisUrl: "redis://unused-in-test",
      redisClient: { publish: publishMock },
    ...
    await publisher.publishGoal(event);

    expect(publishMock).toHaveBeenCalledTimes(1);
    expect(publishMock).toHaveBeenCalledWith("goal-events", JSON.stringify(event));
  });

  it("publishGoal() KHÔNG throw khi client.publish() reject (Redis down)", async () => {
    const publishMock = vi.fn().mockRejectedValue(new Error("connection refused"));
    const publisher = new RedisPublisher({
      redisUrl: "redis://unused-in-test",
      redisClient: { publish: publishMock },
    ...
    await expect(publisher.publishGoal(makeGoalEvent())).resolves.toBeUndefined();
  });
});
