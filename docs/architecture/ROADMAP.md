...
- [ ] So sánh cầu thủ bằng AI (khác `ai_player_summary` — user tự chọn 2 cầu thủ bất kỳ, cần `AiUsageLog` cap-theo-user thật vì không tự giới hạn như summary; chưa có UI compare non-AI nào để build lên, cần trang mới hoàn toàn)
...
**Exit criteria:** đọc được tóm tắt trận đấu ✅ và cầu thủ ✅ do AI viết (verify thật với key Gemini thật), chat hỏi được về trận/cầu thủ, so sánh 2 cầu thủ có nhận xét AI trên web — với chi phí có kiểm soát (usage cap hoạt động).
