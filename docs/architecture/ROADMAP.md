...
- [x] **(2026-08-18) Quyết định: bỏ AWS Bedrock, gọi thẳng LLM API trực tiếp** — Bedrock cần mở AWS account thật + xin quyền truy cập model (approval, có thể mất thời gian) + IAM riêng, không có lợi ích thật cho quy mô app này (không cần compliance/data-residency, app chưa dùng AWS cho gì khác — auth đã là Firebase). Package mới `packages/ai-provider` (`LlmProvider` interface + `AnthropicAdapter`/`GeminiAdapter`, mirror chính xác pattern `packages/data-provider`'s `DataProviderAdapter`) — đổi provider chỉ cần thêm adapter mới. Chọn qua `apps/sync-worker/src/ai-provider.ts`'s `createLlmProvider()` (env `LLM_PROVIDER`: `"anthropic"` mặc định | `"gemini"`). Anthropic: `claude-haiku-4-5-20251001` (rẻ nhất). Gemini: `gemini-3.5-flash-lite` — **free tier thật** (Google AI Studio, không thẻ), đã verify hoạt động thật với key thật. `ai_usage_logs` **chưa dùng** cho match/player summary (job hệ thống hoặc tự giới hạn — xem 2 bullet dưới) — sẽ dùng thật ở Chat/player-compare (user-triggered, cần cap theo user).
- [x] **(2026-08-18) `ai_match_summary`**: tự sinh khi match chuyển sang FINISHED — trigger ở **2 nơi độc lập** (`sync-live-matches.ts` — đường nhanh khi match đang được live-poll; `sync-catalog.ts`'s `syncMatches()` — đường chắc chắn qua re-sync định kỳ, không phụ thuộc live-poll có bắt kịp hay không), gọi chung 1 hàm idempotent `generateMatchSummaryIfNeeded()` (`apps/sync-worker/src/match-summary.ts`, guard bằng `AiMatchSummary.matchId` đã `@unique`). **Không dùng Redis Pub/Sub** (khác goal-notifier Phase 2) — phát hiện lẫn xử lý đều nằm trong sync-worker, thêm channel chỉ để tự nói chuyện với mình là thừa. "Không block API" đạt bằng cách không `await` job trong vòng lặp sync chính (`void ...catch()`), không phải tách process. Có script backfill `pnpm --filter @football-app/sync-worker backfill-match-summaries [limit]` cho match FINISHED từ trước khi tính năng này tồn tại. **(2026-08-18) Nâng cấp**: khi match có `MatchEvent` thật (từ `apps/scraper-sofascore`), prompt đưa thêm danh sách bàn thắng/thẻ/thay người kèm tên cầu thủ+phút — summary nêu cụ thể ai ghi bàn thay vì chỉ tỉ số cuối; match không có event data giữ nguyên hành vi cũ. Chỉ áp dụng cho summary sinh **từ giờ trở đi** (không regenerate ~99 summary cũ, quyết định có chủ đích). Hiển thị ở `/matches/[id]` (card "Tóm tắt trận đấu", chỉ hiện khi đã có). Verify thật với key Gemini thật (qua backfill script).
- [x] **(2026-08-18) `ai_player_summary`**: khác match summary ở 2 điểm — (1) không có lifecycle event tương tự "chuyển FINISHED" để tự trigger, chỉ sinh qua script backfill thủ công (`pnpm --filter @football-app/sync-worker backfill-player-summaries [limit]`), KHÔNG on-demand lúc user vào trang (tránh block request 1-5s); (2) stats cầu thủ đổi theo mùa (khác tỉ số trận đã đông cứng) nên dùng **TTL 7 ngày** (regenerate nếu summary cũ hơn) thay vì sinh 1 lần vĩnh viễn — `AiPlayerSummary.playerId` đã thêm `@unique` (trước đó thiếu, khác `AiMatchSummary.matchId`) để `upsert` được. Dựa trên `PlayerStatistics` mùa gần nhất có data (appearances/goals/assists/thẻ), tái dùng `LlmProvider` y nguyên. Hiển thị ở `/players/[id]` (card "Tóm tắt AI"). Verify thật với key Gemini thật.
- [x] **(2026-08-18) So sánh cầu thủ bằng AI**: khác `ai_match_summary`/`ai_player_summary` (job hệ thống, backfill trước) — user tự chọn 2 cầu thủ bất kỳ nên KHÔNG backfill được, bắt buộc **on-demand, đồng bộ trong 1 request có auth** (`POST /players/compare`, `apps/api/src/routes/player-compare.ts`). Đây là route DUY NHẤT trong `apps/api` gọi LLM trực tiếp — phá lệ "`apps/api` không phụ thuộc `@football-app/ai-provider`" có chủ đích (thêm dependency + `apps/api/src/ai-provider.ts` riêng, mirror `apps/sync-worker`) vì đây là hành động user tự bấm nút "So sánh", không phải page load passive như match/player summary đang bảo vệ. **Lần đầu `AiUsageLog` được dùng thật** (`feature: "player_compare"`, cap **20 lượt/user/24h**) — cap chỉ chặn LLM call thật, KHÔNG tính cache hit hay lỗi validate (422 thiếu số liệu/404 không tìm thấy cầu thủ). Model mới `AiPlayerComparison` — quan hệ 2 FK tới `Player` (khác `AiMatchSummary`/`AiPlayerSummary` 1 FK `@unique`) nên cần canonical order (`playerAId`/`playerBId` tự sort trước khi đọc/ghi) + `@@unique([playerAId, playerBId])`, TTL 7 ngày giống `ai_player_summary` (stats cả 2 cầu thủ đổi trong mùa). Web: trang mới `/players/compare` (`PlayerPicker` — picker riêng, khác `SearchBox`, search qua `GET /players?search=` có sẵn), hiện bảng so sánh không-AI + card "Nhận xét AI". Verify thật với key Gemini thật (kết quả đúng số liệu, cache hit đúng khi đảo thứ tự 2 cầu thủ).
- [x] **(2026-08-19) Chat AI**: `chat_history` (đã có schema sẵn, dùng nguyên) — **KHÔNG build `prompt_templates`/embeddings+pgvector piece này**, quyết định có chủ đích: tự check DB thật lúc lên plan chỉ có ~7 dòng prose thật (2 `AiMatchSummary` + 1 `AiPlayerSummary` + 4 `AiPlayerComparison`) — corpus quá nhỏ để vector search có giá trị hơn SQL/substring match, và `pgvector` extension hoàn toàn chưa setup ở Postgres (image thường, không phải bản có pgvector). Đúng nguyên tắc PROJECT_PLAN §7.1 "chỉ thêm hạ tầng khi có nhu cầu thật đo được" — để pgvector cho piece sau khi corpus đủ lớn. Thay vào đó dùng **"RAG-lite" qua SQL**: mỗi tin nhắn quét tên Team/Player xuất hiện như substring (raw SQL `ILIKE`, đẩy scan xuống Postgres — `apps/api/src/chat-retrieval.ts`'s `buildChatContext()`), ghép `TeamStatistics`/`PlayerStatistics` mùa gần nhất + `AiMatchSummary`/`AiPlayerSummary` liên quan thành context đưa vào prompt. Hạn chế đã biết: không fold dấu (cần `unaccent`, không thêm extension mới); chỉ resolve theo tin nhắn MỚI NHẤT (không tích luỹ context cả session — verify thật: hỏi tiếp bằng đại từ "cậu ấy" mà không nhắc lại tên thì AI đúng đắn nói không có thông tin thay vì bịa, thay vì cố gắng suy diễn — đây là tradeoff chấp nhận được để giữ chi phí prompt có kiểm soát theo session dài). `POST /chat/messages` (`apps/api/src/routes/chat.ts`) — route thứ 2 (sau `player_compare`) gọi LLM trực tiếp trong `apps/api`, tái dùng nguyên `createLlmProvider()` đã có, không cần dependency mới. `AiUsageLog` consumer thứ 2 (`feature: "chat"`, cap **30 lượt/user/24h**). `sessionId` group theo `ChatHistory.sessionId` có sẵn trong schema (không cần bảng `ChatSession` riêng) — không truyền thì tự tạo mới (UUID). `GET /chat/sessions`/`GET /chat/sessions/:id/messages` LUÔN filter thêm `userId` (không tin `sessionId` một mình là biên giới quyền truy cập). Verify thật với key Gemini thật: hỏi về cầu thủ có `AiPlayerSummary` thật → trả lời đúng số liệu thật (12 bàn, đúng dữ liệu); hỏi về cầu thủ không có data (Messi, ngoài phạm vi 5 giải) → AI nói đúng "không có thông tin" thay vì bịa.
- [x] **(2026-08-19) Web: trang chat** — `/chat` (`apps/web/src/app/chat/page.tsx`) — khung chat bubble + sidebar liệt kê session cũ + nút "Cuộc trò chuyện mới", cùng UX pattern loading/error với `/compare`. Thêm vào `NavLinks.tsx`.

**Exit criteria:** đọc được tóm tắt trận đấu ✅ và cầu thủ ✅ do AI viết (verify thật với key Gemini thật), so sánh 2 cầu thủ có nhận xét AI trên web ✅ (verify thật với key Gemini thật, usage cap 20/user/24h hoạt động), chat hỏi được về trận/cầu thủ ✅ (verify thật với key Gemini thật, usage cap 30/user/24h hoạt động, retrieval SQL đúng — xem bullet Chat AI) — với chi phí có kiểm soát.
