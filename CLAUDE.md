...
- `LlmProvider` interface (`src/provider.interface.ts`) + `AnthropicAdapter` (`src/adapters/anthropic.adapter.ts`) — mirror chính xác pattern `DataProviderAdapter` ở `packages/data-provider`: constructor `{ apiKey, model?, fetchImpl? }` dùng `fetch` gốc (không thêm SDK/axios), KHÔNG throw ở constructor nếu thiếu `apiKey` (lỗi thật chỉ lộ lúc gọi `generateText()` — cho phép build/test code trước khi có API key thật).
- `apps/sync-worker` chọn qua `createLlmProvider()` (`src/ai-provider.ts`), đọc env `LLM_PROVIDER` (`"anthropic"` mặc định, chỉ có giá trị này lúc này). Set `ANTHROPIC_API_KEY`, tuỳ chọn `ANTHROPIC_MODEL` (mặc định `claude-haiku-4-5-20251001` — rẻ nhất, đủ cho việc tóm tắt kết quả trận đấu, không cần suy luận phức tạp).
...
- Chat/RAG (piece sau, chưa làm) mới cần embedding + pgvector — `ai_match_summary`/`ai_player_summary` không cần. Provider embedding cân nhắc OpenAI `text-embedding-3-small` thay vì Voyage AI (Voyage bị MongoDB mua 2/2025, rủi ro dài hạn cho API độc lập).
